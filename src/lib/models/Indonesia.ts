...
const Indonesia = mongoose.models.Indonesia || mongoose.model(Name, provinceSchema)

Indonesia.getAllProvince = () => {
    return Indonesia.find({}).select('name id -_id').sort({ name: 1 })
}

Indonesia.getProvince = (id: string) => {
    return Indonesia.aggregate([
        {
            $match: { id: id },
        },
        {
...
Indonesia.getRegency = (id: string) => {
    return Indonesia.aggregate([
        {
            $unwind: '$regencies',
        },
...
Indonesia.getDistrict = (id: string) => {
    return Indonesia.aggregate([
        {
            $unwind: '$regencies',
        },
...
Indonesia.getVillage = (id: string) => {
    return Indonesia.aggregate([
        {
            $unwind: '$regencies',
        },
